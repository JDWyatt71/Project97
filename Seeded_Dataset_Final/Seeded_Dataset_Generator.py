...
def Skilled_type(level, game_version): # skilled: efficient, low retries, bigger funnel
    ...
    enemy_total_moves = clamp(int(round(total_player_moves * (0.90 + 0.25 * factor))), 10, 60)
    ...
    damage_per_hit = add_variance(2.5 + level * 1.2 * factor, spread=0.20) 
    mitigation = 0.20
...
def compute_hp_remaining(level: int, damage: float) -> int:  # HP starts at 50 and passively increases +5 or +15 each level.
    ...
    hp_regen = random.choice(HP_REGEN_CHOICES)     # 5 or 15
...
def table_builder(num_of_sessions): #   called to make both tables
    for i in range(num_of_sessions):
        player_type = random.choice(['Aggressive','Cautious','Inexperienced','Skilled'])
        session_builder(player_type)

def session_builder(player_type): # called to make info for one player
   ...
   for i in range(num_of_runs):
      run_id = unique_num()
      fight_table_builder(player_type, run_id, session_id)

def fight_table_builder(player_type, run_id, session_id): 
    game_version = random.choice(['Easy','Normal','Hard'])
    loss_probs, win_prob = probs_by_type[player_type]
